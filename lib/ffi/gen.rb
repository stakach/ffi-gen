...
class FFI::Gen
  ...
  class StructOrUnion < Type
    ...
    attr_reader :fields, :oo_functions, :written, :generator, :types
  ...
  class FunctionOrCallback < Type
    attr_reader :name, :parameters, :return_type, :function_description, :return_value_description, :is_callback
  ...
  class Writer
    ...
    def puts(*lines)
      lines.each do |line|
        @output << "#{@current_indentation}#{line}".rstrip + "\n"
      end
    end
  ...
  class PointerType < Type
    attr_reader :pointee_name, :depth, :generator

    def initialize(pointee_name, depth, generator)
    ...
    def aliases
      @generator.declarations_by_type.select { |k, v| v.respond_to?(:name) && v.name }.values.map(&:name).map(&:raw)
    end
  end
  ...
  attr_reader :module_name, :ffi_lib, :headers, :prefixes, :output, :cflags, :declarations_by_type, :declarations_by_name
    ...
    when :pointer, :incomplete_array, :block_pointer
      if is_array
        ...
        if type.nil?
          ...
          type = PointerType.new pointee_name, pointer_depth, self
          type
        end
